...
const source = readFileSync(fileURLToPath(new URL('./FolderPickerSheet.vue', import.meta.url)), 'utf8')

describe('FolderPickerSheet runtime API error localization wiring', () => {
  it('uses locale-reactive shared API error copy for formatApiError call sites', () => {
    expect(source).toContain('const copy = computed(() => folderPickerCopy(locale.value))')
