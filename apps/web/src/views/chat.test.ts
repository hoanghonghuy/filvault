...
describe('chat surface contract', () => {
  ...
  it('keeps Chat wallpaper, peek and action copy locale-safe without Vietnamese fallbacks', () => {
    const i18n = readSrc('../lib/i18n.ts')
    for (const key of ['loadingMessages', 'wallpaperPreviewQuestion', 'wallpaperPreviewReply']) {
      expect(i18n.match(new RegExp(`${key}:`, 'g'))).toHaveLength(2)
    }

    expect(chat.match(/\{\{ t\.wallpaperPreviewQuestion \}\}/g)).toHaveLength(2)
    expect(chat.match(/\{\{ t\.wallpaperPreviewReply \}\}/g)).toHaveLength(2)
    expect(chat).toMatch(/\{\{ t\.loadingMessages \}\}/)
    expect(chat).not.toMatch(/t\.[A-Za-z0-9_]+ \|\| '[^']+'/ )
    expect(chat).not.toMatch(/t\.value\.[A-Za-z0-9_]+ \|\| '[^']+'/ )
    expect(chat).not.toMatch(/Giao diện chat trông thế nào|Rất đẹp và dễ nhìn|Đang tải tin nhắn…/)
  })
