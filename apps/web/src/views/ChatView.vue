...
import { chatThemeName, chatWallpaperName, chatCustomWallpaperLabel, chatStorageWallpaperFallback } from '@/lib/chatPersonalizationCopy'
...
const chatThemes = [
  { id: 'blue', color: '#0084ff', gradient: 'linear-gradient(135deg, #0084ff 0%, #0099ff 100%)' },
  { id: 'purple', color: '#8b5cf6', gradient: 'linear-gradient(135deg, #8b5cf6 0%, #ec4899 100%)' },
  { id: 'emerald', color: '#10b981', gradient: 'linear-gradient(135deg, #059669 0%, #10b981 100%)' },
  { id: 'rose', color: '#f43f5e', gradient: 'linear-gradient(135deg, #f43f5e 0%, #fb7185 100%)' },
  { id: 'amber', color: '#f59e0b', gradient: 'linear-gradient(135deg, #d97706 0%, #f59e0b 100%)' },
  { id: 'cyan', color: '#06b6d4', gradient: 'linear-gradient(135deg, #0891b2 0%, #06b6d4 100%)' },
  { id: 'indigo', color: '#6366f1', gradient: 'linear-gradient(135deg, #4f46e5 0%, #6366f1 100%)' },
  { id: 'coral', color: '#f97316', gradient: 'linear-gradient(135deg, #ea580c 0%, #fb923c 100%)' },
  { id: 'slate', color: '#64748b', gradient: 'linear-gradient(135deg, #475569 0%, #64748b 100%)' },
] as const
...
async function selectPhotoFromPersonalStorage(item: TimelineItem) {
  ...
  const customItem: ChatCustomWallpaper = {
    id: generateUUID(),
    url: photoUrl,
    name: item.name || chatStorageWallpaperFallback(locale.value),
    source: 'storage',
    sourceId: item.id,
    createdAt: new Date().toISOString(),
  }
...
const currentWallpaperPresetName = computed(() => {
  if (!threadWallpaper.value || threadWallpaper.value === 'none') return chatWallpaperName(locale.value, 'none')
  const preset = CHAT_WALLPAPER_PRESETS.find((p) => p.id === threadWallpaper.value)
  if (preset) return chatWallpaperName(locale.value, preset.id)
  return chatCustomWallpaperLabel(locale.value)
})
              ...
              <div class="theme-picker-row">
                ...
                <div class="theme-dots">
                  <button
                    ...
                    :title="chatThemeName(locale, th.id)"
                    :aria-label="chatThemeName(locale, th.id)"
          ...
          <div class="wallpaper-presets-section">
            ...
            <div class="wallpaper-presets-grid">
              ...
              >
                ...
                <span class="wallpaper-preset-name">{{ chatWallpaperName(locale, wp.id) }}</span>
              </button>
            </div>
          </div>
        </div>
          ...
          <div class="wallpaper-presets-section">
            ...
            <div class="wallpaper-presets-grid">
              ...
              >
                ...
                <span class="wallpaper-preset-label">{{ chatWallpaperName(locale, wp.id) }}</span>
              </button>
            </div>
          </div>
        </div>
