...
describe('ForgotPasswordView', () => {
  ...
  it('shows the same non-enumerating success presentation after a reset request', async () => {
    ...
    expect(apiMock).toHaveBeenCalledWith('/auth/password/forgot', expect.objectContaining({ method: 'POST', body: JSON.stringify({ email: 'user@example.com' }) }), { auth: false })
  ...
  it('submits one reset request while pending and returns to Login with success feedback state', async () => {
    ...
    const pending = new Promise<void>((resolve) => { resolveReset = resolve })
    ...
    expect(apiMock).toHaveBeenCalledWith('/auth/password/reset', expect.objectContaining({ method: 'POST', body: JSON.stringify({ token: 'valid-reset-token', newPassword: 'password-123' }) }), { auth: false })
    resolveReset()
    await flushPromises()
  ...
  it('reacts to Vietnamese locale across both phases while preserving privacy-safe status copy', async () => {
    setLocale('vi')
    const { wrapper } = await mountRecovery()
    expect(wrapper.get('h1').text()).toBe('Đặt lại mật khẩu')
    expect(wrapper.get('button[type="submit"]').text()).toBe('Gửi hướng dẫn đặt lại')
    await enterResetPhase(wrapper)
    expect(wrapper.get('h1').text()).toBe('Nhập mã đặt lại mật khẩu')
    expect(wrapper.get('[role="status"]').text()).toContain('Nếu tồn tại tài khoản với email user@example.com')
    setLocale('en')
    await wrapper.vm.$nextTick()
    expect(wrapper.get('h1').text()).toBe('Enter your reset token')
    wrapper.unmount()
  })

  it('localizes invalid-token recovery without exposing raw API text', async () => {
    setLocale('vi')
    apiMock.mockRejectedValueOnce(new ApiError('VALIDATION_ERROR', 'raw internal reset detail', 400))
    const { wrapper } = await mountRecovery()
    await wrapper.get('button.token-ready-btn').trigger('click')
    await wrapper.get<HTMLInputElement>('#recovery-token').setValue('expired-token')
    await wrapper.get<HTMLInputElement>('#recovery-password').setValue('password-123')
    ...
    await wrapper.get('form').trigger('submit')
    await flushPromises()
    expect(wrapper.get('[role="alert"]').text()).toContain('không hợp lệ hoặc đã hết hạn')
    expect(wrapper.text()).not.toContain('raw internal reset detail')
    wrapper.unmount()
  })
})
